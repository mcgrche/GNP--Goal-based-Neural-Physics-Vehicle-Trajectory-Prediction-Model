...
class MultihHeadAttention(nn.Module):
    def __init__(self, d_model, h, dropout=0.1, islinear=True):
        ...
        assert d_model % h == 0 # 判断声明语句，这里的d_model就是之前的embed_size，是config文件中表示hidden layer的dimension的
    ...
    def forward(self, query, key, value, mask=None):
        ...
        key = self.w_key(key).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # [B h N d_k], [批次大小, 头数, 序列长度, 每头的特征维度]
        value = self.w_value(value).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # [B h N d_k], [批次大小, 头数, 序列长度, 每头的特征维度]

        x, self.atten = attention(query, key, value, mask, self.dropout)

        x = x.transpose(1, 2).contiguous().view(batch_size, -1, self.h * self.d_k) # [B K d_model],综合各个头独立关注的不同信息子空间
